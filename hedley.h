...
#endif
#if defined(HEDLEY_ISPC_VERSION)
#define HEDLEY_ISPC_VERSION_CHECK(major, minor, patch)                                             \
    (HEDLEY_ISPC_VERSION >= HEDLEY_VERSION_ENCODE(major, minor, patch))
#else
...
#define HEDLEY_IS_CONSTANT(expr) __builtin_constant_p(expr)
    ...
    HEDLEY_TINYC_VERSION_CHECK(0, 9, 24) || HEDLEY_ISPC_VERSION_CHECK(1, 23, 0)
#if defined(__INTPTR_TYPE__)
#define HEDLEY_IS_CONSTEXPR_(expr)                                                                 \
    __builtin_types_compatible_p(                                                                  \
        __typeof__((1 ? (void *)((__INTPTR_TYPE__)((expr) * 0)) : (int *)0)), int *)
#else
#ifndef HEDLEY_ISPC_VERSION
#include <stdint.h>
#endif
#define HEDLEY_IS_CONSTEXPR_(expr)                                                                 \
